...
- Recover an existing immutable release only from the current default-branch
  workflow, and advance only an established production ref through an exact,
  non-force fast-forward.
- Validate GitHub Compare status, counts, base, and merge base without relying
  on a nonexistent head field or a pagination-sensitive commit list.
