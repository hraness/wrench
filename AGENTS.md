...
- Follow `docs/publishing.md` for the interactive npm bootstrap and later stage-only trusted publishing. A monotonically greater stable version reaching `main` must automatically enter the exact staging pipeline; an unchanged version must no-op, and manual dispatch remains the current-`main` recovery path. Keep the minimal OIDC job behind the main-only `npm-stage` environment with no required GitHub deployment reviewers, and bind npm trust to that exact environment and stage-only action. CI must stage automatically after verification. Human inspection and two-factor approval of the npm stage remain mandatory before the version becomes public. Verify that exact public artifact before creating its tag. Treat a `v*` tag as a release request, not a completed release. Keep it equal to `v<package.json version>` on the package commit in `main`; the read-only release gate must rebuild and compare the public npm tarball before its write-scoped job creates the GitHub Release. Verify that Release is non-draft, immutable, and Latest before the workflow fast-forwards the established `website-production` branch to that exact tag commit without force. Keep the write-scoped job limited to Release and ref mutation. Require bounded read-only jobs to bracket the promotion with authenticated GitHub server time, fingerprint the complete current state and `latestStatus` of at most 500 Production deployments, and exhaustively audit only the pinned candidate's REST status history. Bind one exact successful Vercel Production deployment plus stable terminal tag, Release, Latest, workflow source, ref, inventory, and status readbacks before the Release workflow succeeds. Do not make the global receipt depend on previous deployment statuses that GitHub deletes after 90 days; GitHub preserves the current status on the deployment. The only manual branch-write exception is the documented one-time Vercel bootstrap at the exact current immutable Latest Release commit. After it, checked-in policy must keep the Release workflow as the sole routine writer, while live branch enforcement remains a separately reviewed canary-and-ruleset control, and a missing production branch is a hard failure. Keep Vercel's Production Branch on `website-production`; `main` and pull requests are preview sources, never production website sources.
