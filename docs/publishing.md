...
helper and remove only this temporary documentation and test delta. Retain the
canary at `C`, the mirrored permanent rulesets, and the production-only freeze.
Remove that freeze later only by its captured ID after a fresh release-owner
audit; uncertainty leaves production safely frozen.
