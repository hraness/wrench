...
| X | `x` | Official OAuth | `body`, optional `media` and aligned alt-text fields; leave `made_with_ai` unset or `false` unless the user explicitly asked to label attached media | Documented API post |
...
- When the user supplied the copy, never mark the post as AI-generated. Follow [X AI disclosure](x-ai-disclosure.md). Leave official `x` `made_with_ai` unset or `false`. `x-web` `posts.publish` has no such input field.
- Prefer a Wrench transport over the X composer. If a composer fallback is used, require Content disclosure Made with AI OFF before Post, then open the live permalink. A sparkle label means that target failed.
