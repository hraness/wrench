...
- `linkedin-web articles.draft.save@2` is the one observed LinkedIn web operation in current bundle 1.6.0. It binds the numeric member subject to the normalized Article-author profile, creates or replaces one exact private draft, supports paragraphs/H1/H2 plus native HTTPS links, and verifies the exact unpublished editor-response readback. Its fixed API/write and server-response readback exchange runs inside contained Chrome to preserve the bound device session without DOM automation; callers cannot select requests or scripts. Create uses title-shell and content dispatches; exact replacement uses one conservative desired-state dispatch, skips already-matching fields, and never calls a publish or share route.
- Every other LinkedIn web operation remains capture-required. Explicit inert reservations cover inbox folders, one member profile, one organization Page, one bounded page of recommended connections, one connection invitation, Article reads/publication, and other post/comment/message surfaces. Their retained candidates do not confer executable internal requests.
