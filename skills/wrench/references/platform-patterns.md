...
- `linkedin-web articles.draft.save@1` is the one observed LinkedIn web operation in current bundle 1.5.0. It binds the numeric member subject to the normalized Article-author profile, creates or replaces one exact private draft, supports paragraphs/H1/H2 plus native HTTPS links, and verifies the exact unpublished readback. Its fixed internal-API exchange runs inside contained Chrome to preserve the bound device session without DOM automation; callers cannot select requests or scripts. Its two-dispatch create/title-content schedules never call a publish or share route.
