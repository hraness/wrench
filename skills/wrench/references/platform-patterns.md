...
- The strict direct XRPC runtime live-proves bounded feed, post, thread, and media projections against the selected signed-in DID. `posts.publish@3` additionally publishes exact text with at most one plan-bound JPEG/PNG/WebP image, retains the response-bound URI/CID and media identity before readback, verifies the authoritative PDS record, and then polls only the exact public AppView post. `media.publish@1` separately reserves a bounded MP4 video-service upload/processing/record path and remains capture-required. `content.delete@1` is limited to one current-account post URI plus confirmed CID, uses authoritative PDS pre-read and `swapRecord`, and verifies exact `RecordNotFound`. DM reads and every other mutation remain capture-required until exact nonempty fixtures or authorized write evidence exist.
