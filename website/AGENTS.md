...
- `production-release-verifier.ts` – streaming, bounded production checks that bind package version, checked-out HEAD, remote tag, canonical npm, and the immutable Latest GitHub Release.
- `vercel-build.ts` – Vercel admission that runs external release verification only for production and keeps previews independently buildable.
...
- Keep Vercel System Environment Variables enabled and its Production Branch configured as `website-production`. Fail a Vercel build with missing `VERCEL_ENV`, and require `VERCEL_GIT_COMMIT_REF=website-production` for production. Vercel requires the branch to exist before configuration, so `docs/publishing.md` permits one initial create-ref at the exact current immutable Latest Release commit. After that one-time bootstrap, only the release workflow may create or fast-forward the branch after exact npm verification and immutable Latest Release verification; it must never force or accept divergence. Treat `main` and pull-request deployments as previews. Production builds must verify external release state before reading release identity and provider capabilities from their exact checked-out tag commit; preview builds must not depend on npm or GitHub release availability.
- Require a production Git checkout with a resolvable `HEAD`; never substitute a deployment environment SHA. The production npm manifest check relies on the release workflow's preceding exact source-to-registry tarball comparison and immutable Release gate, so keep those workflow gates and branch promotion inseparable.
