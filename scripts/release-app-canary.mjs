...
export function parseCanaryRevocationReceipt(value) {
  ...
  if (
    ...
    receipt.observationCount > 10 ||
    typeof receipt.propagationObserved !== "boolean" ||
    receipt.propagationObserved !== (receipt.observationCount > 2) ||
...
export function parseCanaryEvidence(value) {
  ...
  return Object.freeze({
    ...
    repositoryId,
    revocation: parseCanaryRevocationReceipt(evidence.revocation),
...
export async function proveCanary(options) {
  ...
  const operation = await options.withToken(options.environment, async (token, app) => {
    ...
    if (revocationCallbackCount !== 1) {
      fail("release App token revocation callback was invoked more than once");
    }
    revocation = parseCanaryRevocationReceipt(value);
  });
  if (!tokenOperationCompleted) fail("release App token operation did not run");
  if (revocation === undefined) {
    fail("release App token revocation callback did not provide one receipt");
  }
