...
describe("single-use release App canary", () => {
  ...
  test("accepts only the shared helper's sanitized revocation receipt", () => {
    const immediate = parseCanaryRevocationReceipt({
      converged: true,
      observationCount: 2,
      propagationObserved: false,
      stableDenials: 2,
    });
    const delayed = parseCanaryRevocationReceipt({
      converged: true,
      observationCount: 10,
      propagationObserved: true,
      stableDenials: 2,
    ...
    for (const malformed of [
      ...
      expect(() => parseCanaryRevocationReceipt(malformed)).toThrow();
    }
  });
  ...
  test("binds exact P to C and current-main B to D edges with the complete protocol", async () => {
    ...
    try {
      ...
      expect(Object.isFrozen(evidence.revocation)).toBe(true);
      ...
      expect(delayedEvidence.revocation).toEqual({
        converged: true,
        observationCount: 10,
        propagationObserved: true,
        stableDenials: 2,
      ...
      })).rejects.toThrow("revocation callback did not provide one receipt");
      ...
      })).rejects.toThrow("revocation callback was invoked more than once");
  ...
  test("keeps the workflow input-free, read-only, temporary, and cleanup-bound", async () => {
    ...
    expect(helper).toContain("release App token revocation callback did not provide one receipt");
    expect(helper).toContain("release App token revocation callback was invoked more than once");
    ...
    expect(guide).toContain("The shared token helper then sends exactly one\nempty-body revocation request");
    expect(guide).toContain("requires two stable nonredirecting HTTP 401\nresponses");
    ...
    expect(guide).toContain("no more than fourteen REST\nrequests");
