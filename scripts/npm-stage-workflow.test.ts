...
`, "utf8");
  ...
  test("documents bootstrap, verification, stage-only trust, MFA, and tag ordering", async () => {
    ...
    for (const required of [
      ...
      "exactly mirror the production layers",
...
describe("single-use release App canary", () => {
  ...
  test("binds exact P to C and current-main B to D edges with the complete protocol", async () => {
    ...
    try {
      ...
      const evidence = await proveCanary({
        ...
        ) => Promise<unknown>, onRevoked: (
          receipt: Record<string, unknown>,
        ) => Promise<void>) {
          tokenOperation = true;
          const result = await operation("fixture-token", {
            appId: 700,
      ...
      expect(Object.isFrozen(evidence.releaseAppRevocation)).toBe(true);
      const encodedEvidence = encodeCanaryEvidence(evidence);
      expect(decodeCanaryEvidence(encodedEvidence)).toEqual(evidence);
      const delayedEvidence = parseCanaryEvidence({
        ...evidence,
        releaseAppRevocation: {
          converged: true,
          observationCount: RELEASE_APP_REVOCATION_OBSERVATION_OFFSETS_MILLISECONDS.length,
          propagationObserved: true,
          stableDenials: 2,
        },
      });
      const encodedDelayedEvidence = encodeCanaryEvidence(delayedEvidence);
      expect(decodeCanaryEvidence(encodedDelayedEvidence)).toEqual(delayedEvidence);
      expect(delayedEvidence.releaseAppRevocation).toEqual({
        converged: true,
        observationCount: RELEASE_APP_REVOCATION_OBSERVATION_OFFSETS_MILLISECONDS.length,
        propagationObserved: true,
        stableDenials: 2,
      ...
      for (const malformed of [
        ...
        },
        {
          converged: true,
          observationCount:
            RELEASE_APP_REVOCATION_OBSERVATION_OFFSETS_MILLISECONDS.length + 1,
          propagationObserved: true,
          stableDenials: 2,
        },
        {
        ...
        expect(() => parseCanaryEvidence({
          ...evidence,
          releaseAppRevocation: malformed,
        })).toThrow("canary revocation receipt");
      }
      ...
      function successfulCanaryPush(
        file: string,
        args: readonly string[],
        options: unknown,
      ): ReturnType<typeof spawnSync> {
        if (args.includes("push")) {
          return args.some(value => value === `${fixture.targetSha}:refs/heads/website-production-canary`)
            ? { error: undefined, status: 0, stderr: "", stdout: "ok P..C\n" }
            : { error: undefined, status: 1, stderr: "stale info\n", stdout: "" };
        }
        return cachedTopologySpawn(file, args, options);
      ...
      await expect(proveCanary({
        api: new CanaryApiFixture(callbackStates),
        coordinate,
        cwd: fixture.directory,
        environment,
        preflightReceipt: receipt,
        spawnImplementation: successfulCanaryPush,
        async withToken(_environment: Record<string, string>, operation: (
          token: string,
          identity: Record<string, unknown>,
        ) => Promise<unknown>) {
          return operation("fixture-token", tokenIdentity);
        },
      })).rejects.toThrow("release App revocation receipt is missing");
      ...
      await expect(proveCanary({
        ...
        async withToken(_environment: Record<string, string>, operation: (
          ...
          const result = await operation("fixture-token", tokenIdentity);
          const revocation = {
            converged: true,
            observationCount: 2,
            propagationObserved: false,
          ...
          await onRevoked(revocation);
          await onRevoked(revocation);
          return result;
        },
      })).rejects.toThrow("release App revocation receipt was emitted more than once");
      ...
      })).rejects.toThrow("canary revocation receipt is malformed");

      await expect(proveCanary({
        api: new CanaryApiFixture([callbackStates[0]]),
        coordinate,
        cwd: fixture.directory,
        environment,
        preflightReceipt: receipt,
        spawnImplementation: successfulCanaryPush,
        async withToken(_environment: Record<string, string>, _operation: (
          token: string,
          identity: Record<string, unknown>,
        ) => Promise<unknown>, onRevoked: (
          receipt: Record<string, unknown>,
          ...
          throw new Error("token operation must not begin after an early callback");
        },
      })).rejects.toThrow(
        "release App token revocation callback preceded operation completion",
      );
  ...
  test("keeps the workflow input-free, read-only, temporary, and cleanup-bound", async () => {
    ...
    expect(workflow.match(/node \.\/scripts\/release-app-canary\.mjs prove/gmu) ?? [])
      .toHaveLength(1);
    expect(workflow).toContain("Prove exact canary lease and revocation convergence");
    expect(workflow).not.toContain("Prove exact canary lease and revoked-token denial");
    ...
    expect(workflow).not.toContain("pull_request:");
    expect(helper).toContain("RELEASE_APP_REVOCATION_OBSERVATION_OFFSETS_MILLISECONDS");
    ...
    expect(helper).toContain("release App revocation receipt is missing");
    expect(helper).toContain("release App revocation receipt was emitted more than once");
    expect(helper).toContain("releaseAppRevocation");
    expect(helper).not.toContain("export function parseReleaseAppRevocationReceipt");
    expect(helper).toContain("WRENCH_RELEASE_APP_CANARY_EVIDENCE_V2=");
    expect(helper).not.toContain("WRENCH_RELEASE_APP_CANARY_EVIDENCE_V1=");
    expect(helper).not.toContain("assertRevokedTokenCannotBeReused");
    expect(helper).not.toContain("https://api.github.com/installation/repositories");
    expect(helper).not.toContain("fetchImplementation");
    expect(helper).not.toContain("revokedToken");
    expect(helper).not.toContain("readBoundedBytes");
    expect(helper).not.toContain("TOKEN_REUSE_TIMEOUT_MILLISECONDS");
    expect(helper).toContain("release App token operation did not run");
    expect(helper).toContain("release App token revocation callback preceded operation completion");
    expect(helper.match(/options\.withToken\(/gmu) ?? []).toHaveLength(1);
    expect(helper.match(/\}, async \(value\) => \{/gmu) ?? []).toHaveLength(1);
    ...
    expect(guide).toContain("as `releaseAppRevocation` in its canonical v2 evidence");
    ...
    expect(guide).toContain(
      "The shared token helper then sends exactly one\nempty-body HTTP 204 revocation request",
    );
    expect(guide).toContain("shared ten-slot, 30-second maximum observation window");
    expect(guide).toContain("requires two stable nonredirecting\nHTTP 401 responses");
    expect(guide).toContain("sanitized, immutable\nrevocation receipt through `onRevoked`");
    expect(guide).toContain("never captures the raw token\nfor a separate reuse request");
    expect(guide).toContain("complete App path at no more than fourteen REST requests");
