...
describe("X authenticated internal-API runtime", () => {
  ...
  test("fails closed when the current SearchTimeline query ID drifted", async () => {
    ...
    expect(message).toContain("query-ID drift");
    expect(message).toContain("SearchTimeline:query");
    expect(message).toContain("reviewed evidence is stale");
    expect(message).not.toContain("ChangedQueryId_12345");
    expect(message).not.toContain("hz_94eVAtrtQo_vO3my7Rw");
  });
