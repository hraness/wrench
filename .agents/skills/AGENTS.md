...
- Mark every reusable baseline skill with `metadata.internal: true` so public `skills add hraness/wrench` discovery exposes only the product-owned `wrench` skill.
